	...
	namespace GUI
	{
		void Menu::OnUpdate(const en::UpdateArgs& args)
		{
			if (en::colision::Colide::PointVsRect(args.m_pos, button))
			{
				...
				if (args.mouse.isPress(BASE_MOUSE_BUTTON_1))
				{
					button_action(args);
				}
			}
		...
		void Menu::OnRender(const en::RenderArgs& args, std::shared_ptr<en::render::Text> font)
		{
			...
			args.render.DrawQuad(glm::vec3(out_box.pos,3.0f), out_box.size, { 0.2f,0.5f,0.9f,0.5f });
			font->RenderText(args, out_text, txt_out_pos.x, txt_out_pos.y, 0.50f, out_text_color);

			args.render.DrawQuad(glm::vec3(button.pos, 3.0f), button.size, { 0.2f,0.6f,0.2f,0.5f });
		...
		void Menu::AdjustPositions(std::shared_ptr<en::render::Text> font)
		{
			...
			txt_out_pos = glm::vec3((out_box.pos + (out_box.size / 2.0f)) - (font->PreviewWid(out_text,0.50f) / 2.0f),3.0f);
			txt_out_pos.y = glm::lerp(out_box.pos.y, out_box.pos.y + out_box.size.y, 0.5f);
			txt_button_pos = glm::vec3((button.pos + (button.size / 2.0f)) - (font->PreviewWid(button_text,0.50f) / 2.0f),3.0f);
